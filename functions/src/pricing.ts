import { PRICING, PROMO_CODES } from './config.js';
...
export function priceOrder(subtotal: number, promoCode?: string | null): Totals {
  const { freeDeliveryThreshold, deliveryFee, serviceFee } = PRICING;
...
/** Unknown codes are worth nothing; validating the code itself is separate. */
...
export function isValidPromoCode(promoCode: string): boolean {
  return Boolean(PROMO_CODES[promoCode.trim().toUpperCase()]);
}
